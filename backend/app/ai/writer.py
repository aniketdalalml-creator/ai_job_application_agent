...
from backend.app.ai.models import CompanyResearch, Critique, FitAnalysis
...
- Emphasize matched skills and transferable strengths from the fit analysis
- Do not claim proficiency in missing required skills; address gaps honestly or omit them
...
important JD requirements. Flag invented coverage of missing required skills from the
fit analysis. If the letter is solid, set hasIssues to false and use empty lists."""
...
- Do not invent coverage of missing required skills
- Keep cover letter around 250-350 words
- Return only the JSON object (no markdown fences, no commentary)"""


def _fit_prompt_block(fit: FitAnalysis | None) -> str:
    if not fit:
        return ""
    matched = ", ".join(fit.matched_skills) or "none listed"
    missing_required = ", ".join(fit.missing_required_skills) or "none"
    missing_preferred = ", ".join(fit.missing_preferred_skills) or "none"
    strengths = "; ".join(fit.strengths) or "none listed"
    gaps = "; ".join(fit.gaps) or "none listed"
    return f"""
Candidate fit analysis (do not invent experience to cover gaps; emphasize transferable strengths):
- Overall: {fit.recommendation} at {fit.overall_score:.0f}
- Matched skills to emphasize: {matched}
- Missing required skills: {missing_required}
- Missing preferred skills: {missing_preferred}
- Strengths: {strengths}
- Gaps: {gaps}
- Experience: {fit.experience_assessment or "n/a"}
"""
...
Return only the JSON object. Flag invented coverage of missing required skills."""
...
def write_cover_letter(
    ...
    """
    draft = draft_cover_letter(research, job_description, resume_text, fit)
    critique = critique_cover_letter(job_description, resume_text, draft, fit)
